...
3. **Container contract** (`container`): the built image must `EXPOSE 8080`, run as a non-root `USER`, and answer `GET /healthz` with 200 within 90s of `docker run` (the smoke gate). The non-root check refuses a missing `USER`, `root`, `0`, and `0:`/`root:` forms, and resolves a named user through the image's own `/etc/passwd` (no readable `/etc/passwd` fails closed). The image the gates scan on a release tag is the exact image that deploys, pushed by digest; the deploy never rebuilds your Dockerfile.
