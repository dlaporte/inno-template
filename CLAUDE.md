...
7. **SAST** (`semgrep`): OWASP-top-ten patterns across the whole repository except the platform-owned root `src/`, so `app/`, `lib/`, this repo's `Dockerfile`, `.github/workflows/deploy.yml` and any root-level script are all scanned (string-built HTML, raw SQL formatting, a workflow that passes `secrets: inherit`, etc.).
